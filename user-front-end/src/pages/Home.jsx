...
const Home = () => {
  const { imgFile } = useContext(imgContext);
  ...
  const [userId, setUserId] = useState(() => {
    ...
    return userIdFromQuery || storedUserId;
  });
  ...
  useEffect(() => {
    ...
    } else if (!userIdFromQuery && userId) {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        const response = await axios.post('http://localhost:5000/api/auth/get_user_profile', { id: userId });
        // console.log("Response data:", response.data); // Debugging
        if (response.data.success && response.data.user) {
          setFirstName(response.data.user.firstName);
          setLastName(response.data.user.lastName);
        } else {
          console.warn('User not found or other error:', response.data);
        }
      } catch (error) {
          ...
          <div className="flex flex-col md:flex-row gap-5 w-[100%] px-2">
            <div className="rounded border-gray-400 border-[1px] flex flex-col  pt-2 w-full md:w-1/2">
              <section className="flex px-2 py-2">
                ...
                <div className="flex justify-center items-center bg-slate-500 border-8 rounded-[50%] border-gray-800 md:w-64 w-48 h-28 md:h-[118px]  p-0 m-0 mt-10">
                  <img
                    src="./Assets/Group 9.png"
                    className=" w-48 md:w-[220px] h-[80px]"
                    alt=""
                  />
                </div>
              </section>
